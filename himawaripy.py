...
# Width and height each tile (full image will be width*level by height*level)
width = 550
height = 550
...
def async_download(url_format, latest, x, y, png):
    with urlopen(url_format.format(level, width, strftime("%Y/%m/%d/%H%M%S", latest), x, y)) as tile_w:
        tiledata = tile_w.read()
    tile = Image.open(BytesIO(tiledata))
    # PIL.Image.paste seems to be thread safe, no need to lock
...
def main():
    ...
    # Create a list to hold the threads and a lock to protect
    threads = []
    for x in range(level):
        for y in range(level):
            threads.append(Thread(target = async_download, args = [url_format, latest, x, y, png]))
            threads[x * level + y].start()
    # Wait for everyone to finish
    i = 1
    for thread in threads:
        thread.join()
        print("Downloading tiles: {}/{} completed".format(i, level*level), end="\r")
        i += 1
    print("\nDownloaded\n")
